            ...
            friendRequests.length === 0 ? (
                ...
                friendRequests.map((friendRequest) => (
                    <div key={friendRequest.senderId} className='flex gap-4 items-center'>
                        <UserPlus className='text-lightinline dark:text-darkinline border-lightinline dark:border-darkinline hover:border-lightinteractive dark:hover:border-darkinteractive' />
                        <p className='font-medium text-lg text-darkgray dark:text-gray'>{friendRequest.senderEmail}</p>
                        <button onClick={() => handleAcceptFriendRequest(friendRequest.senderId)} aria-label='accept friend' className='w-8 h-8 bg-correct ring-1 hover:ring-lightinteractive dark:hover:ring-darkinteractive grid place-items-center rounded-full transition hover:shadow-md' >
                            <Check className='font-semibold text-gray w-3/4 h-3/4' />
                        </button>
                        <button onClick={() => handleRejectFriendRequest(friendRequest.senderId)} aria-label='reject friend' className='w-8 h-8 bg-wrong ring-1 hover:ring-lightinteractive dark:hover:ring-darkinteractive grid place-items-center rounded-full transition hover:shadow-md' >
                            <X className='font-semibold text-gray w-3/4 h-3/4' />
                        </button>
                    </div>
                ))
            )
